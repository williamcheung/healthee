...
def main():
    ...
    providers_ddf = dd.read_parquet(PROVIDERS_PATH)
    providers_by_state = providers_ddf.groupby('PROV_CLINIC_STATE')['PROV_KEY'].count().compute().reset_index()

    fig_provider_map = px.choropleth(providers_by_state,
                                     locations='PROV_CLINIC_STATE',
                                     locationmode="USA-states",
                                     color='PROV_KEY',
                                     scope="usa",
                                     title='Provider Availability by State',
                                     labels={'PROV_KEY': 'Number of Providers'},
                                     color_continuous_scale="Viridis"
                                     )
    fig_provider_map.write_html(f'{HTML_OUT_DIR}/provider_availability_by_state.html')
